...
如果向一个磁盘每次写512字节（可以理解sector单位，如果是HHD的话，最小写的粒子单位就是sector大小，但文中所用磁盘是SSD，SSD没有sector概念，只有page粒子大小的写，但可以用512字节作为一个最小计数单位去衡量所有磁盘的读写），但连续写。由于磁盘最小的单位是page，一般是4096字节，因此对于每个page的第一个写，在内部磁盘IO时，会导致出现一个读。为了减少这个读，优化的程序在每8个写的第一个写时，用了4096个字节而不是512字节，因此减少了这个读，从而提高了40%的性能。
